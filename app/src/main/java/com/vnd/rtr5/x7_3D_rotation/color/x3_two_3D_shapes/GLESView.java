package com.vnd.rtr5.x7_3D_rotation.color.x3_two_3D_shapes;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, GestureDetector.OnDoubleTapListener, GestureDetector.OnGestureListener {
    ...
    private final int[] vbo_color_pyramid = new int[1];
    private final int[] vao_pyramid = new int[1];
    private final int[] vbo_position_cube = new int[1];
    private final int[] vbo_color_cube = new int[1];
    ...
    private void initialize(GL10 gl) {
        ...
        // step5 rewind to position 0
        // no sizeof() operator in java, hence given 4 below - sizeof(float)
        ByteBuffer byteBufferPyramidColor = ByteBuffer.allocateDirect(pyramid_color.length * 4);
        byteBufferPyramidColor.order(ByteOrder.nativeOrder());
        FloatBuffer floatBufferPyramidColor = byteBufferPyramidColor.asFloatBuffer();
        floatBufferPyramidColor.put(pyramid_color);
        floatBufferPyramidColor.position(0);
        // now use
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, pyramid_color.length * 4, floatBufferPyramidColor, GLES32.GL_STATIC_DRAW);
        ...
        // step5 rewind to position 0
        // no sizeof() operator in java, hence given 4 below - sizeof(float)
        ByteBuffer byteBufferCubeColor = ByteBuffer.allocateDirect(cube_color.length * 4);
        byteBufferCubeColor.order(ByteOrder.nativeOrder());
        FloatBuffer floatBufferCubeColor = byteBufferCubeColor.asFloatBuffer();
        floatBufferCubeColor.put(cube_color);
        floatBufferCubeColor.position(0);
        // now use
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, cube_color.length * 4, floatBufferCubeColor, GLES32.GL_STATIC_DRAW);
        GLES32.glVertexAttribPointer(VertexAttributesEnum.AMC_ATTRIBUTE_COLOR, 3, GLES32.GL_FLOAT, false, 0, 0);
        GLES32.glEnableVertexAttribArray(VertexAttributesEnum.AMC_ATTRIBUTE_COLOR);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);
