package com.vnd.rtr5.x8_texture.x3_two_3D_shapes;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, GestureDetector.OnDoubleTapListener, GestureDetector.OnGestureListener {
    ...
    private final int[] vbo_texture_coordinates_pyramid = new int[1];
    ...
    private final int[] texture_stone = new int[1];
    private final int[] texture_kundali = new int[1];
    ...
    private void initialize(GL10 gl) {
        ...
        final float[] pyramid_texture_coordinates = {
                // front
                0.5f, 1.0f, // front-top
        ...
        GLES32.glGenBuffers(1, vbo_texture_coordinates_pyramid, 0);
        GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, vbo_texture_coordinates_pyramid[0]);
        // prepare pyramid vertices array for glBufferData
        // step1 enough bytebuffer allocate
        // step2 set byteorder
        // step3 treat as float buffer
        ...
        // step5 rewind to position 0
        // no sizeof() operator in java, hence given 4 below - sizeof(float)
        ByteBuffer byteBufferPyramidTextureCoordinates = ByteBuffer.allocateDirect(pyramid_texture_coordinates.length * 4);
        byteBufferPyramidTextureCoordinates.order(ByteOrder.nativeOrder());
        FloatBuffer floatBufferPyramidTextureCoordinates = byteBufferPyramidTextureCoordinates.asFloatBuffer();
        floatBufferPyramidTextureCoordinates.put(pyramid_texture_coordinates);
        floatBufferPyramidTextureCoordinates.position(0);
        // now use
        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, pyramid_texture_coordinates.length * 4, floatBufferPyramidTextureCoordinates, GLES32.GL_STATIC_DRAW);
    ...
    private void display() {
        ...
        Matrix.rotateM(modelViewMatrix, 0, pAngle, 0.0f, -1.0f, 0.0f);
    ...
    public void uninitialize() {
        ...
        if (vbo_texture_coordinates_pyramid[0] > 0) {
            GLES32.glDeleteBuffers(1, vbo_texture_coordinates_pyramid, 0);
            vbo_texture_coordinates_pyramid[0] = 0;
        }
        if (vao_pyramid[0] > 0) {
