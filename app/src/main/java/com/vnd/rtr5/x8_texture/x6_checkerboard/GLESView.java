package com.vnd.rtr5.x8_texture.x6_checkerboard;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, GestureDetector.OnDoubleTapListener, GestureDetector.OnGestureListener {
    ...
    private final int[] texture_checkerboard = new int[1];
    /**
     * @noinspection unused, FieldCanBeLocal
     */
    private final Context context;
    private final byte[] checkImage = new byte[checkImageWidth * checkImageHeight * 4];
    ...
    private int loadGLTexture() {
        makeCheckImage(); // checkImage array is populated here
    ...
    private void display() {
        ...
        Matrix.translateM(modelViewMatrix, 0, 0.0f, 0.0f, -4.0f);
        {
            ...
            // now use
            GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, square_position.length * 4, floatBufferSquarePosition, GLES32.GL_DYNAMIC_DRAW);
            GLES32.glVertexAttribPointer(VertexAttributesEnum.AMC_ATTRIBUTE_POSITION, 3, GLES32.GL_FLOAT, false, 0, 0);
            GLES32.glEnableVertexAttribArray(VertexAttributesEnum.AMC_ATTRIBUTE_POSITION);
            GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);
        ...
        // square 2
        {
            ...
            // step5 rewind to position 0
            // no sizeof() operator in java, hence given 4 below - sizeof(float)
            ByteBuffer byteBufferSquarePosition = ByteBuffer.allocateDirect(square_position1.length * 4);
            byteBufferSquarePosition.order(ByteOrder.nativeOrder());
            FloatBuffer floatBufferSquarePosition = byteBufferSquarePosition.asFloatBuffer();
            floatBufferSquarePosition.put(square_position1);
            floatBufferSquarePosition.position(0);
            // now use
            GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, square_position1.length * 4, floatBufferSquarePosition, GLES32.GL_DYNAMIC_DRAW);
            GLES32.glVertexAttribPointer(VertexAttributesEnum.AMC_ATTRIBUTE_POSITION, 3, GLES32.GL_FLOAT, false, 0, 0);
            GLES32.glEnableVertexAttribArray(VertexAttributesEnum.AMC_ATTRIBUTE_POSITION);
            GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);
